...
- 状态：🟡 部分生效（P0 分发解耦 + 监听器隔离、P1 心跳看门狗已落地；P2–P3 规划中）
...
- **P2（规划）**：HTTP 重试退避不再在 poll 线程 `Thread.sleep`（改 scheduler 异步）。
- **P3（规划）**：断线检测触发 `OnDisconnectListener` + 清理 `reconnect*` / io 线程池死配置（属对外 API 面，删前逐一确认调用点）。
