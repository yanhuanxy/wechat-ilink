...
```xml
<dependency>
    <groupId>io.github.yanhuanxy</groupId>
...
```groovy
implementation 'io.github.yanhuanxy:wechat-ilink-sdk:3.0.0'
```

> ⚙️ 运行环境要求 **JDK 17+**（自 3.0.0 起；2.x 系列为 JDK 8）。

---

## 🔄 从 2.x 升级到 3.0.0

| 变更项 | 2.x（JDK 8） | 3.0.0（JDK 17） |
|--------|-------------|-----------------|
| JDK 基线 | 8 | **17**（可用 var/record/sealed/text block） |
| Maven groupId | `io.github.lith0924` | **`io.github.yanhuanxy`** |
| artifactId / 包名 | `wechat-ilink-sdk` / `com.github.wechat.ilink.sdk` | 不变 |
| 自动重连 | `ILinkConfig.reconnect*`/`autoReconnect` | 移除（已废弃；产品不支持，断线由消费方处理） |

升级步骤：

1. JDK 升 17+；
2. 依赖坐标改 `<groupId>io.github.yanhuanxy</groupId>`（artifactId / 版本写法不变）；
3. 移除对 `reconnect*` / `autoReconnect` 配置项的引用（已无效）；
4. 收消息仍由消费方自持循环调用 `getUpdates()`，心跳仅做 liveness（见 [ADR-0001](docs/adr/0001-no-reactive-incremental-dispatch-decoupling.md)）。

> 仍需 JDK 8？沿用 `io.github.lith0924:wechat-ilink-sdk:2.3.3`（JDK8 收尾版本，保留在 Maven Central）。
