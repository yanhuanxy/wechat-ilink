...
### P2 — 工程化补齐
**为什么**：覆盖率 75.9%（P2 补 Farm 排行 + 持久层测试后）仍低于 80% 红线，`McpClient` 34% 待可注入 transport；无 CI，质量门禁缺失。
...
| 整体覆盖率 | 75.9%（P2 补 Farm 排行 + 持久层测试后；`McpClient` 34% 待 Phase 3 可注入 transport） | Phase 3 CI + McpClient 可测化 |
