...
**为什么**：Phase 3 走了静态白名单路线；逐次审批（持久子进程 + ApprovalBuffer + y/n 拦截）仍延后。
- 依赖未文档化的 stream-json 控制协议，需先 spike 确认 headless 下 `--permission-prompt-tool` 行为。
...
| 项 | 现状 | 处置 |
|----|------|------|
| `task/ClaudeCodeProvider.java` | 429 行，超 ≤400 行规则 | 拆分（stream-json 解析 / 参数构造 / 进程管理分离） |
