...
### P4 — Phase 3.2 交互式工具审批
**为什么**：Phase 3 走了静态白名单路线；逐次审批仍延后。
- ✅ 已 spike（GO）：`--permission-prompt-tool` 在 headless `-p` 下同步阻塞等待外部决策，可用"外部审批桥"实现，**无需**持久子进程改造。见 [claude-bridge-phase3.2-spike.md](design/claude-bridge-phase3.2-spike.md)。
- 待办：稳定模型 endpoint 后实证精确 JSON schema，再以最小切片实现 ApprovalBuffer + y/n 拦截 + opt-in 开关。
