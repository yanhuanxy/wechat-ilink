...
> **Prereqs:** JDK 17+, Maven 3.6.3+. **Run:** `mvn clean package && java -jar target/wechat-ilink-bot-1.0.0-SNAPSHOT.jar`, then scan the QR code with WeChat. **Zero-config taste:** the Farm mode (`#帮助`) and Chat echo work with **no API key**. **License:** MIT.
> 下方为中文主体文档。
...
- **SDK 是 Maven 依赖**（`io.github.lith0924:wechat-ilink-sdk:3.0.0`，坐标见 [Maven Central](https://central.sonatype.com/artifact/io.github.lith0924/wechat-ilink-sdk)），`mvn` 会自动拉取，**无需克隆 SDK 源码**。
...
| SDK | `io.github.lith0924:wechat-ilink-sdk:3.0.0`（Maven Central） |
...
- **JDK 17**（源码/目标 17；可用 Java 17 语法）。下载：[Adoptium / Temurin](https://adoptium.net/)。
